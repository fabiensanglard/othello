...
void Sauver_Config( int game_mode, int difficulty){
  ...
  // 1�re partie : g�n�ration d'un fichier---------------------------
	// OUVERTURE DU FICHIER en �criture/cr�ation, mode binaire
	...
	// ECRITURE de donn�es dans le fichier
	// �criture d'une chaine de caractere :
		if ( fwrite( ch, strlen(ch), 1, f1 )!=1 )
		{
		printf(" erreur d'�criture de la chaine %s\n", ch);
		//	exit(1);
		}


      
	// �criture de la structure de sauvegarde
		if ( fwrite( &cfg, sizeof(laconfig), 1, f1 )!=1 )
	{
	printf(" erreur d'�criture de la structure\n");
		// exit(1);
	}
	
	//		if ( fprintf( fichiervaleur, sizeof(int), 1, f0 )!=1 )
	//	{
	//	printf(" erreur d'�criture du nombre\n");
		// exit(1);
	//	}
...
void Restaure_Config( int *game_mode, int *difficulty){
	...
	*difficulty=2;
	...
	// POSITIONNEMENT sur le d�but de la structure
	// On se positionne � partir du d�but du fichier, 
	// apr�s la chaine de caract�re qui a �t� �crite
	// et apr�s le tableau d'entiers longs
	...
	// allocation du buffer recevant les donn�es
    
    cfgbuf = (laconfig *)malloc( sizeof(laconfig) );

// lecture � la position courante du fichier
