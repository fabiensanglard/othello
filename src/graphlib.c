...
GC Zone; 
char message[] = "Fenêtre graphique";
int myscreen, Ecran;
...
myhint.width = XSize; myhint.height = YSize;
...
if (x2 >= x1 && y2 >= y1)
	{ x0 = x1; y0 = y1; dx = x2 - x1; dy = y2 - y1;}
		...
		else 	{ x0 = x2; y0 = y1; dx = x1 - x2; dy = y2 - y1;}
...
void cliquer_xy(int *x, int *y){
  ...
  while(myevent.type!=ButtonPress){
    ...
    switch(myevent.type){
    case Expose : XCopyArea(mydisplay,doublebuffer,mywindow,mygc,myevent.xexpose.x,myevent.xexpose.y,myevent.xexpose.width,myevent.xexpose.height,myevent.xexpose.x,myevent.xexpose.y);
...
/*  e.i.    : indifférent
    e.f     : le mode graphique est initialisé (la machine tracé est
          utilisable).
 */
{
    if ((x<0)||(x>XSize))
