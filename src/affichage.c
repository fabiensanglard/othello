...
void Affichage_Parametre_Joueur(int x, int y){
  ...
  write_gr(x+60,y+440,"Modifier la difficulté");
  set_black();  
...
void Effacer_mode_ia( int mode){
 ...
 switch(mode){
 case 2 :    fill_rectangle(133,223,137,227); break;
  case 1 :   fill_rectangle(133,243,137,247);  break;
  case 0 :   fill_rectangle(133,263,137,267);  break;
  default : break;
  }
}
...
void Afficher_OptionMC(int choix){
 ...
 if(choix==0)
     write_gr(x+5,y+275," Plus d'options");
     else  write_gr(x+5,y+275,"Moins d'options");
     }
