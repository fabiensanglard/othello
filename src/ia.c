...
// Utilis� dans l IA intermediaire
...
// Donne une note a la partie pass�e en parametre, cette note est positive si le damier est favorable au noir sinon negative (utilis� dans min-max)
// On pond�re le damier
...
int Coup_Ordi(int *x, int *y, int difficulte)
{
  ...
  case 0:
    ...
    { // On verifie le premier angle
      *x = 0;
      *y = 0;
      CaseTrouve = 1;
    }

    if (!CaseTrouve && Possibilite[7][7] == 1)
    { // On verifie le 2eme angle si le 1er n'est pas jouable
      *x = 7;
      *y = 7;
      CaseTrouve = 1;
    }

    if (!CaseTrouve && Possibilite[0][7] == 1)
    { // On verifie le 3eme angle si le 2eme n'est pas jouable
      *x = 0;
      *y = 7;
      CaseTrouve = 1;
    }

    if (!CaseTrouve && Possibilite[7][0] == 1)
    { // On verifie le 3eme angle si le 4eme n'est pas jouable
      *x = 7;
      *y = 0;
      CaseTrouve = 1;
    }
